...
- 4タブ構成（フィード / URLで要約 / AIツール / マイページ）。モバイルは下部ナビのみ（上部タブは表示しない）、タブ切り替えはタップのみ（左右スワイプでの切り替えは、フィード内の日付チップ列などの横スクロールUIと誤反応するため廃止）。**マイページ**は保存済み・学習マップ・模擬面接・リポジトリチェック・共有ページの5セクションをまとめたハブタブ（機能が増えてもトップレベルのタブ数を増やさない設計）
...
- **音声ポッドキャスト（通勤・通学モード）**: フィード上部に1日1本、その週の注目記事をGeminiのマルチスピーカーTTSが2人の掛け合い形式で読み上げる音声プレイヤーを表示。手や目を離せない移動中でもキャッチアップできる。運営者キーで1日1回だけ生成・Vercel Blobに保存する共有コンテンツ（`AiToolPick`と同じコスト方針）で、同じ日に複数回リクエストされても再生成しない。台本は文字起こしとしても閲覧可能。プレイヤーには取り上げた記事の出典・タイトル・概要も常に表示（音声を聞き逃しても内容を追える）。台本は各記事の出典・タイトルを明示してから内容に入る構成にし、TTSにも「通勤中に聞き取りやすいよう、ゆっくり・明瞭に」という指示を渡している（音声のみに情報を頼らせない設計）。
