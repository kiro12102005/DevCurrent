...
export function MonthlySummaryShare() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t.common.generationFailedError);
      const b = await res.blob();
      setBlob(b);
      setImageUrl(URL.createObjectURL(b));
    } catch {
      setError(t.monthlySummaryShare.generationFailedRetry);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleShare() {
    ...
    if (typeof navigator.share === "function" && navigator.canShare?.({ files: [file] })) {
      try {
        await navigator.share({ files: [file], title: t.monthlySummaryShare.shareTitle });
      } catch {
  ...
  return (
    <div className="rounded-xl bg-white dark:bg-gray-900 shadow-sm border border-gray-200 dark:border-gray-800 p-4">
      <h3 className="font-bold text-gray-800 dark:text-gray-100 mb-1 flex items-center gap-1.5">
        <ImageIcon className="w-4 h-4" strokeWidth={2.25} /> {t.monthlySummaryShare.title}
      </h3>
      <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed mb-3">{t.monthlySummaryShare.description}</p>
      ...
      {!imageUrl ? (
        ...
        >
          {loading ? t.common.generating : t.monthlySummaryShare.generateButton}
        </button>
      ) : (
        <div className="flex flex-col gap-3">
          ...
          <img src={imageUrl} alt={t.monthlySummaryShare.imageAlt} className="w-full max-w-xs mx-auto rounded-lg shadow-sm" />
          <div className="flex gap-2 justify-center">
            ...
            >
              <Download className="w-3.5 h-3.5" strokeWidth={2.25} /> {t.common.save}
            </button>
            <button
              type="button"
              onClick={handleShare}
              className="inline-flex items-center gap-1.5 rounded-full bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 px-4 py-2 text-xs font-semibold"
            >
              <Share2 className="w-3.5 h-3.5" strokeWidth={2.25} /> {t.monthlySummaryShare.shareButton}
            </button>
          </div>
        </div>
      )}
